    ...
    <div className="flex h-full flex-col gap-6 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10 w-full">
          ...
          items.map((item, index) => {
            ...
            return (
              <button
                ...
                className="group relative w-full rounded-3xl border border-[#E4E4ED] bg-white/90 p-6 text-left shadow-sm transition hover:-translate-y-1 hover:shadow-xl cursor-pointer overflow-hidden"
              >
                ...
                <div className="pointer-events-none absolute bottom-[-25%] right-[2%]">
                  {isCamera ? (
                    ...
                    <UploadCloud className="h-24 w-24 sm:h-32 sm:w-32 text-accent-teal/10" />
                  )}
                </div>

                <div className="relative flex items-center justify-between gap-4">
                  <div className="space-y-1">
                    <span className="inline-flex items-center gap-2 rounded-full bg-brand-gold/20 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wide text-primary">
      ...
      {detailItem && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-3xl bg-white shadow-2xl border border-[#E4E4ED] p-6 sm:p-8 relative">
            <button
              type="button"
              onClick={handleCloseDetail}
              className="absolute right-4 top-4 rounded-full p-2 text-[#6B7280] hover:bg-[#F3F4F6] transition"
              aria-label="Tutup"
            >
              <X className="h-4 w-4" />
            </button>
            ...
            <div className="rounded-2xl border border-dashed border-[#E4E4ED] bg-[#F9FAFB] px-4 py-6 text-center text-sm text-[#6B7280]">
            ...
            <div className="mt-6 flex justify-end">
              <button
                type="button"
                onClick={handleCloseDetail}
                className="rounded-2xl border border-[#D4D4D8] px-4 py-2 text-sm font-medium text-[#1F2937] hover:bg-[#F3F4F6] transition"
              >
                Tutup
              </button>
            </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-3xl bg-white shadow-2xl border border-[#E4E4ED] p-6 sm:p-8">
